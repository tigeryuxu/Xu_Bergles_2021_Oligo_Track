...
for fileNum = 1:length(natfnames)
    ...
    for i = 1:length(sortedmat)
        ...
        elseif cur_idx == cur_frame
            ...
            prev_cell_idx = cur_cell_idx - 1;
            j = find(sortedmat(:, 5) == prev_cell_idx);
                        
            %% skip if last trace had a cell
            prev_frame = sortedmat(j, 1);
            ...
            raw_next = im2double(timeseries_RAW(:, :, :, cur_idx));   %% get previous frame
            
            
            cc_cur = all_cc(cur_idx + 1);
            cc_next = all_cc(cur_idx);
...
%             
            ...
            if match_1 == 1 && match_2 == 0
                ...
                cd(foldername)
                cd('./mip quality check NEW CELLS/')
                ...
                seg_next = im2double(timeseries_SEG(:, :, :, cur_idx));
                ...
                %% SAVE:
                cd(foldername)
                cd('./Training cell track full auto NEW CELLS');
                if exist(strcat('./', save_name), 'dir') == 0
                    mkdir(save_name)
                end
                cd(save_name);
